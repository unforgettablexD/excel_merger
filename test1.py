...
# Setup logging
logging.basicConfig(filename='file_joiner.log', level=logging.INFO)
...
selected_files = []
column_dropdowns = []
TEMP_FILE_PATH = "temp_joined_data.xlsx"
...
def display_data(df):
    ...
    merge_more_button = tk.Button(preview_window, text="Merge More Files with this Data", command=lambda: merge_additional_files(df))
    merge_more_button.pack(pady=10)
...
join_type_dropdown = ttk.Combobox(root, textvariable=join_type_var, values=["inner", "outer", "left", "right"])
join_type_dropdown.pack(pady=5)

process_files_button = tk.Button(root, text="Join Files", command=process_files)
